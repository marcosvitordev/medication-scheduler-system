...
export class SchedulingService {
  ...
  async buildAndPersistSchedule(
    ...
    let entries: WorkingEntry[] = prescription.items.flatMap((item) => {
    ...
    const persisted = await this.scheduledDoseRepository.save(
      entries.map((entry) =>
        this.scheduledDoseRepository.create({
          prescription,
